%% LyX 2.2.1 created this file.  For more info, see http://www.lyx.org/. KI
...
\begin{abstract}
Geo-temporal visualization of Twitter search results is a challenging task since the simultaneous display of all matching tweets would result in a saturated and unreadable display. Thus, the development of novel visual interfaces is necessary to help focus a user's attention by filtering information relevant to their query. In this work, we propose a Visual Twitter Information Filtering (Viz-TIF) tool for query-driven optimization and ranking of spatial, temporal, and content-based filters to help focus a user's exploration of search results.  We leverage a fast greedy optimization algorithm to optimize an approximation of expected F-Score metric to generate these filters and demonstrate its application to search 2 years of Twitter content, specifically for a user's information need related to natural disasters occurring in the US.  Our demo shows that Viz-TIF is capable of extracting geo-temporally coherent filters given search queries, thus aiding the user in visually searching and browsing social network content and enabling new opportunities for the application of information retrieval and optimization techniques to general visual information exploration interfaces.

\noindent {\bf Keywords:}  Visual Search Interfaces; Information Filtering; Constrained Optimization.
\end{abstract}
\maketitle
...
To this end, we propose a Visual Twitter Information Filtering (Viz-TIF)\footnote{\url{http://118.138.233.203:8080/AUI4IRSearch/}}
...
\begin{centering}
\subfigure[Global display showing results.]{\includegraphics[width=4.2cm]{imgs/map}\label{Fig:GlobalDisplay}} \subfigure[Filtered and focused search interface.]{\includegraphics[width=4.2cm]{imgs/filtered_map}\label{Fig:FilteredDisplay}} 
\par\end{centering}
...
This task is not clustering since there is a relevance score for each tweet used to optimize F-Score.}
\label{Fig:UseCase} 
\end{figure}
...
To make the task of VID filtering
more concrete, we introduce an example search filtering use case. Consider the case of searching events related to natural
...
related to his/her query, a VID that is aware of a probability
estimate that each tweet is relevant to the query could automatically optimize a ranking of 
filters to visually display relevant tweet content with the
least amount of irrelevant clutter.  
%The user could then more efficiently browse through the subset of tweets provided by the ranked filter settings to carry out investigation w.r.t.\ the original information need while covering a large fraction of the relevant content.
...
tweets to his/her query.  
Further, since filters are equivalent to Boolean retrieval~\cite{Baeza-Yates2010} (they either select or do not select content as relevant) and we have a probabilistic estimate of relevance, we propose to evaluate
\emph{expected} variants of standard precision, recall, and F-score.
...
However, as standard for both precision and recall, we note that they can be trivially optimized through pathological solutions.  That is, the filter that selects all information elements (i.e., all time, all space, no excluded keywords) would trivially maximize (expected) recall.  Similarly, the filter that selects the highest probability singleton information element would maximize expected precision.  This leaves expected F-score as the main non-pathological Boolean objective to balance expected precision and recall and hence is the filter objective we seek to optimize in this work.
%While this diverges from the standard information retrieval setting where ranked documents are chosen individually \cite{Baeza-Yates2010}, these additional constraints do not change the overall information retrieval objective to select relevant information given the user's information need and constraints of the user interface. 
...
To the best of our knowledge, Viz-TIF is the first tool to address
information filtering in VIDs for social media as a ``supervised'' optimization of spatial, temporal, and content-based filter settings w.r.t.\ surrogates of F-Score well-suited to this task.
...
\section{V\lowercase{iz}-TIF architecture and features}

The main components of the architecture of Viz-TIF are: 
\begin{enumerate}
\item \emph{Crawler and Indexer:} of tweets for further
retrieval.
...
\item \emph{Visual Interface:} to present the information to the user in an accessible and interactive form using filters.
\end{enumerate}
...
The filter optimization component of Viz-TIF relies on the optimization
of an approximation of \emph{expected} F-Score metric \cite{Bouadjenek2018b}. This expectation 
%is computed based on the conditional probability $p(R|t,q)$, i.e.,
is based on an estimated conditional probability $p(q|t)$ 
%the probability 
that a tweet $t$ is relevant to the query $q$.  To obtain this probability estimate,
the ranking function component of Viz-TIF uses a language model scoring approach, which assumes that the terms of a query $q$ are ``generated'' by a
...
The filter optimization component of Viz-TIF relies on the configuration of three sub-filters
...
seeking to optimize an approximation of the expected F-Score metric.
...
events and so the user should have a choice of multiple filters to choose from (e.g., Figure~\ref{Fig:FilteredDisplay} provides three different filters to choose from).  To generate an additional candidate filter given a current set of filters,
all selected tweets by the existing filters
have their probability scores zeroed out (since they are already covered by a filter) and the filtering algorithm is run
again, where it will inherently produce a filter for a different content set.
...
\caption{Main interface of Viz-TIF.}
...
selects the number of filters to be returned, and sets an initial value of
$\beta$ for the $F_\beta$-Score used in filter optimization. A low value of $\beta$ constrains the algorithm to focus
...
\begin{figure}[H]
\begin{centering}
\includegraphics[width=8cm]{imgs/main2}
\par\end{centering}
...
Viz-TIF succeeded in finding two different filters that focus on different (distinct) events related to the initial search query.
\end{itemize}
\begin{figure}[H]
\begin{centering}
...
In this paper, we have expanded on the information filtering paradigm of Belkin and Croft \citep{Belkin1992} and developed Viz-TIF, a tool to search Twitter content through spatial, temporal, and content-based filters. Viz-TIF formulates filtering as an optimization problem 
%the probabilistic relevance estimate of tweets
w.r.t.\ a user-provided query.
In particular, Viz-TIF leverages a fast greedy optimization algorithm
to maximize an approximation of the expected F-Score metric to generate and rank multiple filters.  We provided a demonstration use case of Viz-TIF for exploring 2 years of Twitter
content for %efficiently search for
finding information relating to natural disasters.  By treating filter selection for a user's queries as an expected F-Score optimization problem (as opposed to an unsupervised clustering problem that is unaware of visual presentation of filters, or query relevance estimates), Viz-TIF was able to select relevant filters to provide temporal, spatial, and keyword context for distinct events related to the user's queries.
...
of results presentation (e.g., using size or color for visual ranking
emphasis).
