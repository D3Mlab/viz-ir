...
%Thus, clustering retrieved result search can help to scan a few coherent groups of elements than many individual elements. 
%Clusters represent different information perspectives related to a query and supply the user a clear and a quick overview of the search results without necessarily having to browse all matched elements.
...
%Therefore, it is critical to develop relevance-driven clustering methods for Visual Information Displays (VIDs) that are capable of displaying highly relevant clusters for each query.
...
% While certainly true to some extent, we find this a rather passive approach -- it \textit{assumes} that if a document in a cluster is highly relevant, then other documents in the cluster will also be highly relevant. Hence, the question we raise here is: \textit{why making such an assumption when we can optimize for relevance?}
% %Clustering can be used to address this problem~\cite{Manning2008,Jardine1971,Voorhees1985}, but because conventional clustering methods such as K-Means don't exploit the relevance signal, clusters will inherently be flooded with irrelevant elements.
% In this paper, we argue that the development of relevance-driven clustering methods for Visual Information Displays (VIDs) are required to  restrict the display to information relevant to queries using highly relevant clusters showing multiple information perspectives.


\begin{figure}[t]
\begin{centering}
\subfigure[Baseline display showing all results.]{\includegraphics[width=4.07cm]{imgs/baseline}\label{Fig:GlobalDisplay}}\hspace{0.05cm}\subfigure[Visually clustered results display.]{\includegraphics[width=4.07cm]{imgs/bps}\label{Fig:FilteredDisplay}}
\par\end{centering}
...
%users could restrict the information displayed
%through a variety of cluster settings to get a clear overview of individual events \textendash{} by panning and
...
% an optimization problem for cluster of how to restrict cluster settings to show
...
%  To the best of our knowledge, this work is the first to address clustering in VIDs as a ``supervised'' clustering optimization problem. %In brief, we make the following contributions:}
%\begin{enumerate}
