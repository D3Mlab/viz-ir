 ...
 * automatically determine the number of clusters based on BIC scores. Starting
 * with only one cluster, the X-Means algorithm goes into action after each run
 * of K-Means, making local decisions about which subset of the current
 * centroids should split themselves in order to better fit the data. The
 * splitting decision is done by computing the Bayesian Information Criterion
 * (BIC).
 *
 * <h2>References</h2>
 * <ol>
 * <li> Dan Pelleg and Andrew Moore. X-means: Extending K-means with Efficient
 * Estimation of the Number of Clusters. ICML, 2000. </li>
 * </ol>
 *
 * @see KMeans
 * @see GMeans
...
public class XMeans extends KMeans implements Serializable {
     ...
     */
    public XMeans(double[][] data, int kmin, int kmax, double alpha, double beta, double gamma, double maxDistTime, double maxDistPosition) {
        if (kmax < 2) {
        ...
        KMeans kmeansInit = new KMeans(data, kmin, alpha, beta, gamma, maxDistTime, maxDistPosition);

//        System.out.println("*******************");
//        for (int i = 0; i < data.length; i++) {
////            System.out.println(data[i][1] + " " + data[i][2] + " " + kmeansInit.y[i]);
//            System.out.println((data[i][0] - 1356998401000L) / (1420070341000L - 1356998401000L) + " 0 " + kmeansInit.y[i]);
//
//        }
//        System.out.println("*******************");
        size = new int[k];
        ...
        for (int i = 0; i < kmeansInit.getNumClusters(); i++) {
            size[i] = kmeansInit.getClusterSize()[i];
        }

        centroids = kmeansInit.centroids;

        // within-cluster sum of squares
        double[] wcss = new double[k];
        for (int i = 0; i < n; i++) {
//            wcss[j] += Math.squaredDistance(kmeansInit.[i], centroids[0]);
            wcss[y[i]] += Clustering.getD3mDistance(data[i], centroids[y[i]], alpha, beta, gamma, maxDistTime, maxDistPosition);

        }

        distortion = getDistortion(data, alpha, beta, gamma, maxDistTime, maxDistPosition);
...
//                    logger.info("Cluster {} too small to split: {} samples", i, size[i]);
                ...
                kmeans[i] = new KMeans(subset, 2, alpha, beta, gamma, maxDistTime, maxDistPosition);
...
//                System.out.println(k);
                double newDistortion = clustering(data, sums, alpha, beta, gamma, maxDistTime, maxDistPosition);
